...
#### Node instance endpoint: https://humanode.wvm.network

### WeaveVM Archiver node instance info

```bash
curl -X GET https://humanode.wvm.network/v1/info
```
**returns:**
...
```bash
curl -X GET https://humanode.wvm.network/v1/all-networks-info
```
...
```bash
curl -X GET https://humanode.wvm.network/v1/block/$BLOCK_ID
```

### Decode the WVM archived block data for a given EVM block ID (return original block data in JSON format)

```bash
curl -X GET https://humanode.wvm.network/v1/block/raw/$BLOCK_ID
```
